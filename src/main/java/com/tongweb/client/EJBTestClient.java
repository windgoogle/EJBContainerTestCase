...
import com.tongweb.ejbs.QuerySessionRemote;
...
/**
 * Created by huangfeng on 2017/7/5.
 */
public class EJBTestClient {
    ...
    public static void main(String[] args) throws  Exception{
        ...
        QuerySessionRemote remote = (QuerySessionRemote) ctx.lookup(jndiName);
    ...
    private static void excute(QuerySessionRemote remote) throws InterruptedException{
    ...
    private static void initJndi(String propath, Properties props) {
        try {
            props.load(EJBTestClient.class.getClassLoader()
    ...
    private static class Task implements Runnable {
        private QuerySessionRemote remote;
        private static int threadCount = 0;
        private String threadName = String.valueOf(threadCount++);

        public Task(QuerySessionRemote remote) {
            this.remote = remote;
            ...
            try {
                for (int i = 0; requestNum == -1 || i < requestNum; i++) {
                    ...
                    if ("sum".equals(method)) {
                        long result = remote.sum(1,2);
                    }
                    if ("multiply".equals(method)) {
                            remote.multiply(1,2);
                    }
                    if ("testGC".equals(method)) {
                        remote.testGC();
                    }
